...
After increasing our test coverage, which expanded our work to look for syntax errors in more notebooks, we now see a 4.4x speedup, which is consistent with our expectations. 

\begin{center}
%\begin{minipage}{0.7\textwidth}
    \centering
    \includegraphics[width=.7\textwidth]{booktests/parsl_speedup_chart_no_x_lines.png}
    \captionof{figure}{Parallel testing speedup using Parsl against sequential execution of tests.}
...
\scnote{
\begin{enumerate}
    \item Add citation for `testbook`
    
     \item All references and figures need to be referenced in the text. In particular, Figure 5 and reference [3] require in-text citations.
    
     \item How to apply ``Cucumber tests for Behavior-Driven Development'' in QMCPy?

     \item Add a link to the presentation slides and notebooks

    \item add make testbook and add to readme on running juypter notebook tests

    \item workflow fixing to github actions ask IIT person presentation
    \item alexander culler ( german) lookup nvidia mental images , art owen 

    \item Add details about architecture where the speedup were observed.
...
\bibitem[4]{nteract contributors. 2021. testbook: unit testing framework for Jupyter notebooks. version 0.4.2. (2021). https://github.com/nteract/testbook. Accessed: Sept. 23, 2025.}

\bibitem[5]{Cucumber. Cucumber: Write automated tests in plain language. https://cucumber.io (accessed Sept. 25, 2025).}
