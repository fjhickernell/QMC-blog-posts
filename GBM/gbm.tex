...
{\tiny\input{GBM/outputs/gbm_comparison_table.tex}}

\newpage
\lstinputlisting[style=Python, basicstyle=\footnotesize\ttfamily, caption={Function to generate GBM paths with QuantLib.}, label={lst:quantlib}]{GBM/code/quantlib_util.py}
...
path counts. 
\begin{itemize}
\item \LarysaNote{Regarding accuracy, QMCPy's Sobol sampler generally achieves the lowest mean
absolute error (MAE), particularly when a larger number of paths are used,
reaching errors below $10^{-3}$. %QMCPy's Sobol sampler demonstrates remarkable
...
\item Regarding speed, low-discrepancy samplers like Sobol, lattice, and Halton
